...
class FeaturesArray(dict):
    ...
    def get_feature(self, index: int) -> Feature | str:
        ...
        if feature is not None:
            ...
            if response:
                register_number = struct.unpack("!H", response[:2])[0]
                try:
                    feature = Feature(register_number)
                except ValueError:
                    feature = f"unknown:{register_number:04X}"
                self[feature] = index
