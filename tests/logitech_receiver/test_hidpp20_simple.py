...
def test_get_firmware():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.DEVICE_FW_VERSION)
...
def test_get_kind():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.DEVICE_NAME)
...
def test_get_friendly_name():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.DEVICE_FRIENDLY_NAME)
...
def test_get_battery_status():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.BATTERY_STATUS)

    feature, battery = _hidpp20.get_battery_status(device)

    assert feature == Feature.BATTERY_STATUS
...
def test_get_battery_voltage():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.BATTERY_VOLTAGE)

    feature, battery = _hidpp20.get_battery_voltage(device)

    assert feature == Feature.BATTERY_VOLTAGE
...
def test_get_battery_unified():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.UNIFIED_BATTERY)

    feature, battery = _hidpp20.get_battery_unified(device)

    assert feature == Feature.UNIFIED_BATTERY
    assert battery.level == 80
    assert battery.status == common.BatteryStatus.DISCHARGING
...
def test_get_adc_measurement():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.ADC_MEASUREMENT)

    feature, battery = _hidpp20.get_adc_measurement(device)

    assert feature == Feature.ADC_MEASUREMENT
    assert battery.level == 90
    assert battery.status == common.BatteryStatus.RECHARGING
    assert battery.voltage == 0x1000
...
def test_get_battery():
    ...
    feature, battery = _hidpp20.get_battery(device, Feature.BATTERY_STATUS)
...
def test_get_battery_none():
    responses = [
        hidpp.Response(None, 0x0000, f"{Feature.BATTERY_STATUS:0>4X}"),
        hidpp.Response(None, 0x0000, f"{Feature.BATTERY_VOLTAGE:0>4X}"),
...
def test_get_mouse_pointer_info():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.MOUSE_POINTER)
...
def test_get_vertical_scrolling_info():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.VERTICAL_SCROLLING)
...
def test_get_hi_res_scrolling_info():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.HI_RES_SCROLLING)
...
def test_get_pointer_speed_info():
    responses = [hidpp.Response("0102", 0x0400)]
    device = hidpp.Device(responses=responses, feature=Feature.POINTER_SPEED)
...
def test_get_lowres_wheel_status():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.LOWRES_WHEEL)
...
def test_get_hires_wheel():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.HIRES_WHEEL)
...
def test_get_new_fn_inversion():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.NEW_FN_INVERSION)
...
def test_get_host_names(responses, expected_result, mock_gethostname):
    device = hidpp.Device(responses=responses, feature=Feature.HOSTS_INFO)
...
def test_get_onboard_mode():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.ONBOARD_PROFILES)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [
                hidpp.Response(None, 0x0000, f"{Feature.REPORT_RATE:0>4X}"),
...
def test_get_polling_rate(
    ...
    device = hidpp.Device(responses=responses, feature=Feature.EXTENDED_ADJUSTABLE_REPORT_RATE)
...
def test_get_remaining_pairing():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.REMAINING_PAIRING)
...
def test_config_change():
    ...
    device = hidpp.Device(responses=responses, feature=Feature.CONFIG_CHANGE)
